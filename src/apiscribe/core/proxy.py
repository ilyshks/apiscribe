...
class ProxyServer:
    ...
    def __init__(self, config: Config, collector: Collector, daemon):
        self.config = config
    ...
    async def broadcast_log(self, log):
        """Отправляет лог всем подключённым WebSocket-клиентам, удаляя закрытые."""
        to_remove = []
        for ws in self.daemon.log_clients:
            if ws.closed:
                to_remove.append(ws)
                continue
            try:
                ...
                to_remove.append(ws)  # при ошибке тоже удаляем
        # Удаляем нерабочие сокеты
        for ws in to_remove:
            if ws in self.daemon.log_clients:
                self.daemon.log_clients.remove(ws)
    ...
    async def _forward(self, request: web.Request):
        try:
            ...
            return web.json_response({"error": "Target API unreachable"}, status=502)
        except asyncio.TimeoutError:
            return web.json_response({"error": "Target API timeout"}, status=504)
        except aiohttp.ClientError as e:
            return web.json_response({"error": f"Client error: {str(e)}"}, status=502)
        except Exception:
    ...
    async def _process(self, request: web.Request):
        try:
            ...
            async with self.session.request(
                ...
                req_schema = (
                    self.analyzer.generate_schema(req_json) if req_json else None
                )
                resp_schema = (
                    self.analyzer.generate_schema(resp_json) if resp_json else None
                )
